import {
  Pattern,
  patterns,
} from "@api/FillWord/Mock/Infrastructure/Patterns.db";
import { db } from "@api/FillWord/Mock/Infrastructure/Words.db";
import { IFillWord, IFillWordCeil } from "@domain/FillWord/FillWord.models";
import { ID } from "@domain/SharedKernel.types";
import { games } from "@api/FillWord/Mock/Infrastructure/Games.db";
...
export class FillWordCreateService {
  ...
  private static getFlipPattern(width: number): Pattern {
    ...
    const patternNumber: number = Math.floor(
      Math.random() * patterns[width + "x" + width].length
    );
      ...
      word.forEach((symbol) => {
        switch (putCase) {
          case 0:
            ...
            break;
          case 1:
            patternWord[patternWord.length] = [
              width - 1 - symbol[0],
              width - 1 - symbol[1],
            ];
            break;
  ...
  private static getFlipNoSquarePattern(
    ShortSide: number,
    TallSide: number,
    FallOnSide = false
  ): Pattern {
    ...
    const patternNumber: number = Math.floor(
      Math.random() * patterns[matrixSize].length
    );
    const pattern: Pattern = [];
    const rawPattern: Pattern = patterns[matrixSize][patternNumber];
  ...
  private static getNinePattern(): Pattern {
    ...
    if (Math.floor(Math.random() * 4) > 1) {
      ...
      rawPattern.forEach((word) => {
        ...
        word.forEach(
          (symbol) =>
            (patternWord[patternWord.length] = [symbol[0] + 6, symbol[1]])
        );
      });
      ...
      rawPattern.forEach((word) => {
        ...
        word.forEach(
          (symbol) =>
            (patternWord[patternWord.length] = [symbol[0], symbol[1] + 6])
        );
      });
    } else {
      // 5х5, 5х4, 4х5, 4х4
  ...
  public static makeGame(width: number): IFillWord {
    ...
    const idPattern: ID[][] = pattern.map((word) => {
      ...
      const wordFromBase =
        db[wordLengthInBase][
          Math.floor(Math.random() * db[wordLengthInBase].length)
        ];
      const idPattern: ID[] = [];

      for (let index = 0; index < word.length; index++) {
        map[word[index][0]][word[index][1]] = {
          content: `${wordFromBase[index]}`,
          id: id,
        };
        idPattern.push(id);
        id++;
